...
class Zabbix_DB():
    ...
    def __init__(self):
        ...
        self.output = open(self.root + "/zabbix/Output/Zabbix_DB_Config_" + dt + ".txt", "a+")
        ...
        self.api_payload = {"ToolName": "Zabbix", "Component": self.component, "ServerFQDN": self.hostname,
                            ...
                            "Status": "1", "StatusReason": "DB Configuration Initiated"
                            }
    ...
    def config(self):
        try:
            ...
            cmd = 'zcat create.sql.gz | mysql -uzabbix -p'+self.db_pwd+' zabbix'
            ...
            if connection != 'false' and flush_priv !='false' and create_db != 'false' and create_user != 'false' and alter_user != 'false' and grant_priv != 'false' and status == 0 and close_sql != 'false':
                self.logger.info("DB Configuration Success")
                self.api_payload["StatusReason"] = "DB Configuration Success"
                ...
                self.logger.info("DB Configuration Failed")
                self.api_payload["StatusReason"] = "DB Configuration Failed"
