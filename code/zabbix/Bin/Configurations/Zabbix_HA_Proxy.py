...
sys.path.insert(0, os.environ['Admin_Root'] + '/zabbix/ha_script')
from uistatus import sendstatus
from ha_script import config
class Zabbix_DB():
    ...
    def pyzabbix_module(self):
        try:
            self.logger("Installing pyzabbix ...")
            pyzabbix = "pip install pyzabbix"
            os.system(pyzabbix)
            time.sleep(5)
            self.logger("Installing zabbix Config ....")
            conf = "pip install config"
            os.system(conf)
            time.sleep(5)
            self.logger("pyzabix Module Done.")
